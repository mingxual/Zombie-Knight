...
public class RocketLauncherScriptLPFP : MonoBehaviour {
	...
	private void Update () 
	{
		...
		{
			outOfAmmo = true;
			int leftAmmo = WeaponSwitch.instance.rechargeMagazine(ammo);
			if (leftAmmo > 0)
			{
		...
		if (Input.GetMouseButtonDown (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			...
			var projectile = (Transform)Instantiate (
				Prefabs.projectilePrefab, 
				Spawnpoints.bulletSpawnPoint.transform.position, 
				Spawnpoints.bulletSpawnPoint.transform.rotation);

			projectile.GetComponent<ProjectileScript>().damagePoints = damagePoints;
