...
#pragma warning restore 649
        ...
        private bool _isGrounded, _isClimbingLadder;
        ...
        void OnTriggerStay(Collider other)
        {
            if (other.gameObject.tag == "Ladder")
            {
                transform.Translate(input.Strafe * climbSpeed * Time.deltaTime * transform.up);
            }
        }
        ...
        private void Update()
        {
            ...
            Jump();
            PlayFootstepSounds();
            gameObject.GetComponent<Rigidbody>().useGravity = !_isClimbingLadder;
        }
        ...
        {
            float strafe = 0.0f;
            if (!_isClimbingLadder) strafe = input.Strafe;
            var direction = new Vector3(input.Move, 0f, strafe).normalized;
